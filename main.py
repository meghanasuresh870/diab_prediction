...
st.title('Classifiaction Model')
st.write('Diabetes Yes or No?')

def open_page():
    st.sidebar.title('Enter Patient Details :')
    
    preg = st.sidebar.number_input('Pregancies',min_value = 0,max_value = 20)
    gluc = st.sidebar.slider('Glucose',min_value=0,max_value=200)
    bp = st.sidebar.slider('BloodPressure',min_value=0,max_value=140)
    skl = st.sidebar.slider('SkinThickness',min_value=0,max_value=100)
    insulin = st.sidebar.number_input('Insulin',min_value=0,max_value=900)
...
if st.sidebar.button('Submit'):
    loaded_model = pickle.load(open('diab_predic.pkl','rb'))
    loaded_scaler = pickle.load(open('diab_scaler.pkl','rb'))
    st.write(features)
    ...
    if prob >=0.6:
        st.error(' patient have diabetes')
    else:
